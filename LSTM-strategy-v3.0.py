...
def on_init(context):   
    ...
    path='model_2/lst_max.csv'
    lst_max=pd.read_csv(path,index_col=0)
    path='model_2/lst_min.csv'
...
#%%
#自定义函数
#自动改变止损止盈变量,n表示前需要收集前几天的数据,m表示窗口的长度
def loss_profit(context,n,m):
    lst=[]
    ts=pd.DataFrame(columns=['close'])
    df=pd.DataFrame(np.random.randn(1,1),columns=['close'])
    for i in range(n+1)[1:]:
        day=datetime-timedelta(days=i)
        data = context.function.get_market_data(day)
        for j in data:
            df.close=j['close']
            ts=ts.append(df,ignore_index=True)
        if len(data)!=0:
            lst.append(len(data))
    return max_min(ts,lst,m)
    
#计算ts中窗口中最大值和最小值的差距
def max_min(ts,lst,m):
    start=0
    lst_day=[]
    for i in lst:
        lst_diff=[]
        end=start+i
        day=ts.iloc[start:end]
        start+=i
        for j in range(i-m):
            lst_diff.append(float(abs(np.max(day.iloc[j:j+m])-np.min(day.iloc[j:j+m]))))
        lst_day.append(np.max(lst_diff))
    return np.sum(lst_day)/float(len(lst))
#生成lstm    
...
def on_bar(context):
    ...
    if len(temp)>=n_steps:
        ...
        with tf.Session() as sess:
            saver.restore(sess, "model_2/lstm_model.ckpt")
        ...
        if context.var.position==0 and (bar.datetime.hour!=14 and bar.datetime.minute<=45):
            ...
            if bar.datetime.hour==14 and bar.datetime.minute>=55:
                ...
                change=bar.close-context.var.buy
            ...
            if bar.datetime.hour==14 and bar.datetime.minute>=55:
                ...
                change=context.var.short-bar.close
                if change >profit:
                    context.function.cover(bar.close)
                    context.var.position=0
                    context.function.log('买跌平仓')
...
def on_newday(context):
    ...
    #重新计算止损止盈值
    profit=loss_profit(context,2,60)
    loss=-profit
