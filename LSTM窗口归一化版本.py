...
def get_train_data(data,label):
    ...
    for i in lst:
        batch_x.append(data.ix[i:i+T].apply(lambda x:preprocessing.minmax_scale(x)).values.tolist())
...
def get_test_data(data,label,n_test):
    ...
    batch_y=[]
    m=len(data)-T
    lst=np.random.randint(0,m,size=n_test)
    for i in lst:
...
batch_size =  1000          #minibatch选择的大小
display_step = 10
...
with tf.Session() as sess:
    ...
    saver.save(sess,"model-3/lstm_model.ckpt")
#%%
    #测试的数量
