...
def on_init(context):    
    ...
    tar=tarfile.open('model_window_final.tar.gz')
    tar.extractall()
    tar.close()
    ...
    n_input = 22 # 时间序列点的特征数 #去除掉volume和ob指标
    n_steps = 60 # 时间序列的长度
    n_hidden = 240 # 隐藏层结点数
    n_classes = 3 # 标签数量
    #每分钟的基础指标 
    context.var.ts=pd.DataFrame(columns=['open','close','high','low','EMA_5min',
       'EMA_10min', 'EMA_15min', 'EMA_20min', 'RSI', 'upperband',
       'middleband', 'lowerband', 'macd', 'macdsignal', 'macdhist',
       'slowk', 'slowd', 'ATR', 'WILLR', 'SAR', 'DEMA', 'MOM'])
    context.var.pred=[]
    context.var.real=[]
...
def strategy(context,p):
    bar=context.var.bar
    if context.var.position==0 and (bar.datetime.hour!=14 and bar.datetime.minute<=30):
        if p=='可买涨':
...
def on_bar(context):
    ...
    bar=context.var.bar
    #组合得到新的ts
    df=pd.DataFrame(np.random.randn(1,22),columns=context.var.ts.columns)
    df['close']=bar.close
    df['open']=bar.open
    df['low']=bar.low
    df['high']=bar.high
    ...
    if len(temp)>=n_steps:
        with tf.Session() as sess:
            saver.restore(sess, "model_window_final/lstm_model.ckpt")
